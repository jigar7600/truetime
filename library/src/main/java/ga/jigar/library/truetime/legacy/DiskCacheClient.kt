...
internal class DiskCacheClient {
    ...
    fun getCachedSystemTime(): Long = if (cacheUnavailable()) {
        0L
    } else _cacheInterface?.get(CacheInterface.KEY_CACHED_SYSTEM_TIME, 0L) ?: 0L
     ...
     */
    @JvmOverloads
    fun clearCachedInfo(cacheInterface: CacheInterface? = this._cacheInterface) {
    ...
    fun cacheTrueTimeInfo(sntpClient: SntpClient) {
        ...
        _cacheInterface?.put(CacheInterface.KEY_CACHED_SYSTEM_TIME, System.currentTimeMillis())
    }

    fun getCachedBootId(): String {
        val dBootId = ""
        return if (cacheUnavailable()) {
            dBootId
        } else _cacheInterface?.get(CacheInterface.KEY_CACHED_BOOT_ID, dBootId) ?: dBootId
    }

    fun cacheBootId(bootId: String) {
        if (cacheUnavailable()) return
        _cacheInterface?.put(CacheInterface.KEY_CACHED_BOOT_ID, bootId)
    }
